  ...
  fedmeetings_wavg %>% 
    ...
    mutate(date = factor(date)) %>% 
           #date = fct_relevel(as.character(date), {unique(.$date)})) 
    #mutate(date = format(strptime(as.character(date), "%Y-%m-%d"), "%d/%m/%Y"),
           #date = factor(date),
           #date = fct_relevel(date, unique(.$date)),
           #meetings = factor(meeting),
           #meetings = fct_relevel(meeting, c("15/Jun/2022", "27/Jul/2022", "21/Sep/2022",
